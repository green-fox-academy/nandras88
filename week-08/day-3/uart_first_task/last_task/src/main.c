...
int main(void) {
	...
	while (1) {
		...
		HAL_UART_Receive(&UartHandle, character, 3, 500);
		HAL_Delay(1);
		HAL_UART_Transmit(&UartHandle, character, 3, 500);
		if (strcmp(character, "on") == 0) {
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_SET);
			HAL_Delay(500);


			memset(character, '\0', 3);
		} else if (strcmp(character, "off") == 0) {
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_RESET);
			memset(character, '\0', 3);

		} else if (strcmp(character, "\0\0\0") != 0) {
			for (int i = 0; i < 3; i++) {
